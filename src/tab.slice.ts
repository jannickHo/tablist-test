  ...
  reducers: {
    ...
    splitTabListContainerToParent: (
      ...
      if (!action.payload.parentTabListContainerId) {
        ...
        const newTabListContainerParent = _.cloneDeep(
          state.tabListContainerData[action.payload.parentTabListContainerId]
        );
        newTabListContainerParent.tabListContainer.splice(
          directionIndex,
          0,
          tabListContainerId
        );
    ...
    splitTabListContainerToNewTabListContainer: (
      ...
      const tabListId = uuidv4();
      const tabListContainerIdNewTabList = uuidv4();
      const tabListContainerId = uuidv4();
      ...
      const newTabListContainerNewTablist: TabListContainer = {
        tabListId,
        id: tabListContainerIdNewTabList,
        direction: "none",
        tabListContainer: []
      };
      ...
      const newTabListContainer: TabListContainer = {
        ...
        tabListContainer:
          ...
          action.payload.direction === "up"
            ? [tabListContainerId, action.payload.tabListContainerId]
            : [action.payload.tabListContainerId, tabListContainerId]
      };

      state.tabListContainerData = {
        ...state.tabListContainerData,
        [tabListContainerId]: newTabListContainer,
        [tabListContainerIdNewTabList]: newTabListContainerNewTablist
      };
      state.tabListData = { ...state.tabListData, [tabListId]: newTabList };
      ...
      if (!action.payload.parentTabListContainerId) {
        const newTabListContainerList = _.cloneDeep(state.tabListContainerList);
        newTabListContainerList.splice(
          action.payload.index,
          1,
          tabListContainerId
        );
...
export const selectTabListContainerList = (state: RootState) =>
  state.tab.tabListContainerList;

export const selectTabListContainerData = (id: string) => (state: RootState) =>
  state.tab.tabListContainerData[id];
export const selectTabListData = (id: string) => (state: RootState) =>
  state.tab.tabListData[id];
